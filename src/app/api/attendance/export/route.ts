...
export async function GET(request: NextRequest) {
  try {
    ...
    const status = Object.values(AttendanceStatus).includes(requestedStatus as AttendanceStatus) ? requestedStatus as AttendanceStatus : "";
    ...
    if (branchId && !(await db.branch.count({ where: { id: branchId, companyId: tenant.companyId, deletedAt: null } }))) return new Response("Cabang tidak valid", { status: 400 });
    const rowsData = await db.attendance.findMany({ where: { companyId: tenant.companyId, workDate: { gte: start, lt: end }, ...(branchId ? { branchId } : {}), ...(status ? { status } : {}) }, include: { employee: { include: { department: true, position: true } }, branch: true }, orderBy: { employee: { fullName: "asc" } } });
    ...
    await db.auditLog.create({ data: { companyId: tenant.companyId, actorUserId: tenant.session.userId, action: "EXPORT", module: "attendance", newValue: { rows: rowsData.length, date: start.toISOString().slice(0, 10), branchId: branchId || null, status: status || null } } });
