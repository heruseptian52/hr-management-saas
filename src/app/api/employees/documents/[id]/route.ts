...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const tenant = await requirePermission("employees", "view"), { id } = await params;
    const item = await db.employeeDocument.findFirst({ where: { id, companyId: tenant.companyId, deletedAt: null }, select: { fileData: true, mimeType: true, fileName: true, fileSize: true } });
