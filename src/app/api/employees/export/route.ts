...
export async function GET(request: NextRequest) {
  try {
    const tenant = await requirePermission("employees", "export"), q = request.nextUrl.searchParams.get("q")?.trim(), status = request.nextUrl.searchParams.get("status"), ids = request.nextUrl.searchParams.get("ids")?.split(",").filter(Boolean).slice(0, 500);
    ...
    await db.auditLog.create({ data: { companyId: tenant.companyId, actorUserId: tenant.session.userId, action: "EXPORT", module: "employees", newValue: { rows: employees.length, filtered: Boolean(q || status || ids?.length), selected: ids?.length ?? 0 } } });
