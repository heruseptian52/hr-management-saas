...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return NextResponse.redirect(new URL("/organization?error=validation", request.url), 303);
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/organization?saved=${parsed.data.kind}`, request.url), 303);
  } catch {
    return NextResponse.redirect(new URL("/organization?error=duplicate", request.url), 303);
  }
}
