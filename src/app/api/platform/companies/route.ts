...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!parsed.success) return NextResponse.redirect(new URL("/platform?error=validation", request.url), 303);
    const company = await db.$transaction(async transaction => {
      const created = await transaction.company.create({ data: { ...parsed.data, email: parsed.data.email || null } });
      await transaction.role.create({ data: { companyId: created.id, name: "Owner", isSystem: true, permissions: ownerPermissions } });
    ...
    return NextResponse.redirect(new URL(`/platform?saved=${company.code}`, request.url), 303);
  } catch {
    return NextResponse.redirect(new URL("/platform?error=duplicate", request.url), 303);
  }
}
