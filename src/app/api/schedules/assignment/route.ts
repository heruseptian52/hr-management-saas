...
const schema = z.object({ assignmentId: z.string().cuid(), shiftId: z.string().cuid().or(z.literal("OFF")), returnMonth: z.string().regex(/^\d{4}-\d{2}$/) });
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.redirect(new URL(`/schedules?month=${parsed.returnMonth}&saved=manual`, request.url), 303);
