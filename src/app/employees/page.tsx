...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string; q?: string }> }) {
  ...
  return <main className="settings-page"><section className="settings-card"><header><div><span className="eyebrow">PANBOY HR · P2</span><h1>Data karyawan</h1><p>{employees.length} data ditampilkan dari {tenant.membership.company.name}.</p></div><a href="/dashboard">Kembali</a></header>
    ...
    <div className="data-table"><div className="data-row employee heading"><b>NIK</b><b>Nama</b><b>Cabang</b><b>Departemen</b><b>Jabatan</b><b>Status</b></div>{employees.map(employee => <div className="data-row employee" key={employee.id}><strong>{employee.employeeNumber}</strong><span>{employee.fullName}<small>{employee.email ?? ""}</small></span><span>{employee.branch?.name ?? "-"}</span><span>{employee.department?.name ?? "-"}</span><span>{employee.position?.name ?? "-"}</span><span>{employee.employmentStatus}</span></div>)}</div>
