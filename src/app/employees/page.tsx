...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<{ saved?: string; deleted?: string; error?: string; q?: string }> }) {
  ...
  const [employees, branches, departments, positions, contractTypes] = await Promise.all([
    ...
    db.masterData.findMany({ where: { companyId: tenant.companyId, category: "CONTRACT_TYPE", isActive: true, deletedAt: null }, select: { id: true, name: true }, orderBy: [{ sortOrder: "asc" }, { name: "asc" }] }),
  ]);
