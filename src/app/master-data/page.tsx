...
export default async function MasterDataPage({ searchParams }: { searchParams?: Promise<{ error?: string }> }) {
  ...
  return <main className="settings-page"><section className="settings-card"><header><div><span className="eyebrow">MASTER DATA</span><h1>Data Karyawan</h1><p>Kategori dinamis khusus {tenant.membership.company.name}; tidak terlihat oleh perusahaan lain.</p></div><a href="/dashboard">Kembali</a></header>
    {query.error && <div className="form-error">Data master tidak dapat disimpan. Nama yang sama tidak boleh dibuat dua kali.</div>}
    ...
    <div className="master-links"><h2>Organisasi</h2><a href="/organization">Kelola Jabatan, Divisi, dan Cabang</a><a href="/shifts">Kelola Master Shift</a></div>
  </section></main>;
}
