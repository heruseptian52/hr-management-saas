...
export default async function OrganizationPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  return <main className="settings-page"><section className="settings-card"><header><div><span className="eyebrow">PANBOY HR</span><h1>Struktur organisasi</h1><p>Cabang, departemen, dan jabatan milik {tenant.membership.company.name}.</p></div><div className="header-actions"><a className="secondary-button" href="/organization/schedule-rules">Aturan shift departemen</a><a href="/dashboard">Kembali</a></div></header>
