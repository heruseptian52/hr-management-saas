...
export default async function SchedulesPage({ searchParams }: { searchParams: Promise<{ month?: string; branchId?: string; departmentId?: string; saved?: string; error?: string }> }) {
  ...
  const gridRows = [...employeeRows.values()].map(items => ({ employeeId: items[0].employeeId, employeeName: items[0].employee.fullName, cells: items.map(item => ({ id: item.id, day: item.date.getUTCDate(), shiftId: item.shiftId, type: item.type })) }));
  const swapRequests = schedule ? await db.scheduleSwapRequest.findMany({ where: { companyId: tenant.companyId, scheduleId: schedule.id }, orderBy: { createdAt: "desc" }, take: 20 }) : [];
  const assignmentMap = new Map(schedule?.assignments.map(item => [item.id, item]) ?? []);
  const assignmentLabel = (id: string) => { const item = assignmentMap.get(id); return item ? `${item.employee.fullName} • ${item.date.getUTCDate()} ${monthValue} • ${item.type === "OFF" ? "LIBUR" : item.shift?.code ?? "-"}` : "Jadwal tidak tersedia"; };
  return <main className="settings-page"><section className="settings-card schedule-card"><header><div><span className="eyebrow">PANBOY HR</span><h1>Jadwal & shift</h1><p>Jadwal bulanan otomatis yang tetap dapat diedit manual.</p></div><a href="/dashboard">Kembali</a></header>
    ...
    {schedule && schedule.status !== "LOCKED" && hasPermission(permissions, "schedules", "edit") && <details className="swap-panel"><summary>Ajukan swap shift</summary><form action="/api/schedules/swaps" method="post"><input type="hidden" name="action" value="request"/><input type="hidden" name="scheduleId" value={schedule.id}/><input type="hidden" name="returnMonth" value={monthValue}/><label>Jadwal pertama<select name="fromAssignmentId" required>{schedule.assignments.map(item => <option key={item.id} value={item.id}>{assignmentLabel(item.id)}</option>)}</select></label><label>Ditukar dengan<select name="toAssignmentId" required>{schedule.assignments.map(item => <option key={item.id} value={item.id}>{assignmentLabel(item.id)}</option>)}</select></label><label>Alasan<input name="reason" maxLength={300} placeholder="Contoh: kebutuhan keluarga"/></label><button>Ajukan persetujuan</button></form></details>}
    {swapRequests.length > 0 && <section className="swap-list"><h2>Pengajuan swap shift</h2>{swapRequests.map(item => <article key={item.id}><div><b>{assignmentLabel(item.fromAssignmentId)}</b><span>⇄</span><b>{assignmentLabel(item.toAssignmentId)}</b><small>{item.reason || "Tanpa alasan"}</small></div><strong className={`swap-${item.status.toLowerCase()}`}>{item.status}</strong>{item.status === "PENDING" && hasPermission(permissions, "schedules", "approve") && <div className="swap-review"><form action="/api/schedules/swaps" method="post"><input type="hidden" name="action" value="approve"/><input type="hidden" name="requestId" value={item.id}/><input type="hidden" name="returnMonth" value={monthValue}/><input name="reviewNotes" placeholder="Catatan approval" maxLength={300}/><button>Setujui</button></form><form action="/api/schedules/swaps" method="post"><input type="hidden" name="action" value="reject"/><input type="hidden" name="requestId" value={item.id}/><input type="hidden" name="returnMonth" value={monthValue}/><input type="hidden" name="reviewNotes" value="Ditolak oleh approver"/><button className="danger-button">Tolak</button></form></div>}</article>)}</section>}
    ...
    {schedule ? <ScheduleGrid rows={gridRows} shifts={shifts.map(item => ({ id: item.id, code: item.code, name: item.name, color: item.color }))} days={new Date(Date.UTC(year, month, 0)).getUTCDate()} month={monthValue} editable={schedule.status === "DRAFT" && hasPermission(permissions, "schedules", "edit")}/> : <div className="empty-state"><h2>Belum ada jadwal {monthValue}</h2><p>Gunakan tombol Generate atau Salin bulan lalu untuk membuat draft.</p></div>}
