...
export default async function SelfServicePage() {
  ...
  return <main className="settings-page"><section className="settings-card ess-card">
    ...
    {employee && <><div className="ess-profile"><div><small>Nama</small><strong>{employee.fullName}</strong></div><div><small>ID Karyawan</small><strong>{employee.employeeNumber}</strong></div><div><small>Jabatan</small><strong>{employee.position?.name ?? "-"}</strong></div><div><small>Departemen</small><strong>{employee.department?.name ?? "-"}</strong></div><div><small>Cabang</small><strong>{employee.branch?.name ?? "-"}</strong></div><div><small>Status</small><strong>{employee.employeeStatusLabel || employee.employmentStatus}</strong></div></div>
      <div className="ess-grid"><section><h2>Jadwal Bulan Ini</h2>{schedule.map(x => <div className="ess-row" key={x.id}><b>{x.date.toISOString().slice(0, 10)}</b><span>{x.type === "OFF" ? "LIBUR" : x.shift?.name ?? "Kerja"}</span></div>)}{schedule.length === 0 && <p>Belum ada jadwal.</p>}</section><section><h2>Absensi Terakhir</h2>{attendance.map(x => <div className="ess-row" key={x.id}><b>{x.workDate.toISOString().slice(0, 10)}</b><span>{x.status}</span></div>)}</section><section><h2>Cuti & Izin</h2>{leaves.map(x => <div className="ess-row" key={x.id}><b>{x.typeName}</b><span>{x.status} · {x.totalDays} hari</span></div>)}</section><section><h2>Lembur</h2>{overtime.map(x => <div className="ess-row" key={x.id}><b>{x.workDate.toISOString().slice(0, 10)}</b><span>{x.status} · {(x.totalMinutes / 60).toFixed(1)} jam</span></div>)}</section><section><h2>Slip Gaji</h2>{payslips.map(x => <a className="ess-row" key={x.id} href={`/payroll/${x.payrollPeriod.id}/${x.employeeId}`}><b>{x.payrollPeriod.name}</b><span>{x.payrollPeriod.status}</span></a>)}{payslips.length === 0 && <p>Belum ada slip final.</p>}</section><section><h2>Pelatihan Saya</h2>{trainings.map(x => <div className="ess-row" key={x.id}><b>{x.trainingProgram.name}</b><span>{x.trainingProgram.startDate.toISOString().slice(0, 10)} · {x.completionStatus}</span></div>)}{trainings.length === 0 && <p>Belum ada pelatihan.</p>}</section></div></>}
